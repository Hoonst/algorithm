...
def process(graph, shark_info):
    ...
    while king_loc < seconds:
        ...
        #         row_idx = original_row_idx
            ...
            while speed:
                moved_row = row_idx + speed * dx[direction]
                moved_col = col_idx + speed * dy[direction]

                if 0 <= moved_row < rows and 0 <= moved_col < seconds:
                    temp.append([moved_row, moved_col, original_speed, direction, weight])
                    break
